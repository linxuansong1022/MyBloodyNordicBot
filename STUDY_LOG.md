...
- **完成 §4.17 剩余 2 个待决问题**：安全名单（Fail Closed 白名单）+ 摘要 prompt 设计（Tier 2 自由文本 + Tier 3-Lite 结构化 JSON，附完整模板和输入输出示例）。**§4.17 13/13 问题全部关闭**
- **新增 §10「测试与评估策略」章节**（Karpathy 视角 + 评估金字塔 + 失败四分类 + 已实现/未实现评估盘点 + `tests/evals/` 基础设施设计 + 演进路线）
- **改造 qa-tester skill**：从 Python RAG 项目迁移版改成 MiniCode 对齐（保留 6 条普世铁律，删除 Python/Streamlit/MCP 特定部分，对齐 §10 框架）
- **第一次完整走通 auto-coder skill**：实现 `/clear` 命令作为试金石功能

### 🌟 今天最大的认知突破：Skill 触发是语义匹配，不是字面匹配

触发 auto-coder 时，我一开始说「帮我写 code 实现 /clear」—— AI 没触发 skill，因为它在字面匹配 "auto code"。但实际上 auto-coder 的 description 里有兜底条款 "or wants the teaching-style doc-driven flow applied to a concrete task"——**语义上**我的请求完全符合。

结论：**skill 触发是"语义 + 字面"混合匹配**，而且 AI 应该识别意图而不是只认关键词。但作为用户，**显式使用触发词（"auto code"）可以提高命中率**，避免 AI 判断错。以后养成习惯：用明确触发词。

### `/clear` 实现的小收获

- **跨模块信号传递的三种模式**：共享模块（usage-tracker）、callback 注入（/clear 选的这种）、返回值携带 signal。callback 注入对这种小 feature 最轻量
- **发现 codebase 的隐藏不一致**：`index.ts` 用 reassign（`messages = [messages[0]]`），`tty-app.ts` 用 in-place mutate（`args.messages.length = 1`）。两种风格共存，callback 模式优雅地兼容了两者——每个 UI 传入自己的 closure
- **不改不必要的东西**：我没有试图统一这两种状态管理风格，因为这是 scope creep。`/clear` 只解决当下问题

### 下一步

- 用 qa-tester skill 测试 `/clear` 的实现（第一次完整走 auto-coder → qa-tester 两个 skill 的协同）
- 端到端验证：`npm run dev` → 对话 → `/clear` → 再问 agent 之前的内容 → 应该答不上来
- §4.17 的真实代码实现（Tier 1 → Tier 2 → Tier 3-Lite）
